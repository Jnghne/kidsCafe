...
@Tag(name= "Reservation", description = "예약과 관련된 요청을 처리하는 Controller 입니다.")
@RestController
...
public class ReservationController {
    ...
    @Operation(summary = "수업 예약 API", description = "사용자가 특정 매장의 수업 일정을 선택해서 예약을 생성하는 API입니다. " +
                                                    ...
                                                    "\n 3. 예약하려는 수업 일정이 유효한지 검증한다. (과거,당일,15일 이후의 수업은 예약 불가) " +
                                                    "\n 4. 중복 예약 여부를 검증한다. (매장 + 수업 일정 별로 1번만 예약 가능하다)" +
                                                    "\n 5. 수업 일정의 예약 인원 수를 증가시킨다. 만약 수업에 남은 자리가 없다면 예외가 발생한다." +
                                                    "\n 6. 예약 처리를 진행한다.")
    @PostMapping
    ...
    @Operation(summary = "예약 취소 API", description = "사용자가 예약한 수업 일정을 취소하는 API입니다. " +
                                                        ...
                                                        "\n 2. 예약 취소한 만큼 수업의 인원수를 감소한다." +
                                                        "\n 3. 예약 데이터의 예약 상태를 CANCEL로 변경시킨다 (이력 관리 용도와 추후 사용될 여지가 있을 것으로 판단해서 DELETE 처리 X) " )
